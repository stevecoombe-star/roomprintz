-- PI-5G4b: STAGE Partner Portal Product+default-Variant create executor (planVersion 3).
--
-- Adds a create-capable RPC beside frozen G3
-- public.vibode_stage_apply_partner_patch(jsonb) (planVersion 1) and frozen G4a
-- public.vibode_stage_apply_partner_patch_v2(jsonb) (planVersion 2).
-- Does not rewrite v1 or v2. Does not execute rendered SQL text.
-- Does not mutate Assets or Scenes. Does not create Collections.
-- Schema + function only. Do not seed publish rows.

begin;

create or replace function public.vibode_stage_apply_partner_patch_v3(p_apply jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if jsonb_typeof(p_apply->'planVersion') <> 'number' or (p_apply->>'planVersion')::integer <> 3 then
  ...
  if jsonb_array_length(p_apply->'collectionCreates') > 0 then
  ...
  if not v_has_product and not v_has_variant and not v_has_create and not v_has_product_create and not v_has_collection and not v_has_add and not v_has_remove then
...
revoke all on function public.vibode_stage_apply_partner_patch_v3(jsonb)
  from public, anon, authenticated;
grant execute on function public.vibode_stage_apply_partner_patch_v3(jsonb)
  to service_role;
