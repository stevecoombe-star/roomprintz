-- PI-5G4c: STAGE Partner Portal Collection create executor (planVersion 4).
--
-- Adds a Collection-create-capable RPC beside frozen G3
-- public.vibode_stage_apply_partner_patch(jsonb) (planVersion 1), frozen G4a
-- public.vibode_stage_apply_partner_patch_v2(jsonb) (planVersion 2), and frozen G4b
-- public.vibode_stage_apply_partner_patch_v3(jsonb) (planVersion 3).
-- Does not rewrite v1, v2, or v3. Does not execute rendered SQL text.
-- Does not mutate Assets or Scenes. Does not deactivate or hard-delete Collections.
-- Schema + function only. Do not seed publish rows.

begin;

create or replace function public.vibode_stage_apply_partner_patch_v4(p_apply jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if jsonb_typeof(p_apply->'planVersion') <> 'number' or (p_apply->>'planVersion')::integer <> 4 then
...
revoke all on function public.vibode_stage_apply_partner_patch_v4(jsonb)
  from public, anon, authenticated;
grant execute on function public.vibode_stage_apply_partner_patch_v4(jsonb)
  to service_role;
