-- PI-5G5C1: STAGE Partner Portal Product/Variant create executor (planVersion 5).
--
-- Inherits v4 Product/Variant/Collection insert semantics. Changes new
-- Variant Asset eligibility from "an existing Partner Variant already
-- references this Asset" to "Partner↔Asset mapping exists and Asset is
-- ready". Frozen v1–v4 RPCs are not rewritten. Does not execute rendered
-- SQL text. Does not mutate Assets, mappings, storage, or Scenes.
-- Does not retarget existing Variant.current_asset_id.
-- Schema + function only. Do not seed publish rows.

begin;

create or replace function public.vibode_stage_apply_partner_patch_v5(p_apply jsonb)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_has_product_create boolean;
  v_has_collection_create boolean;
...
begin
  ...
  if jsonb_typeof(p_apply->'planVersion') <> 'number' or (p_apply->>'planVersion')::integer <> 5 then
  ...
  if jsonb_typeof(coalesce(p_apply->'productCreates', 'null'::jsonb)) <> 'array'
     or jsonb_typeof(coalesce(p_apply->'variantCreates', 'null'::jsonb)) <> 'array'
     or jsonb_typeof(coalesce(p_apply->'collectionCreates', 'null'::jsonb)) <> 'array'
     or jsonb_typeof(coalesce(p_apply->'productUpdates', 'null'::jsonb)) <> 'array'
  ...
  select name, slug
    into v_partner_name, v_partner_slug
  from public.vibode_stage_partners
  where partner_id = v_partner_id;
  if not found or v_partner_name is null or btrim(v_partner_name) = '' then
    raise exception 'VIBODE_STAGE_PUBLISH:PARTNER_MISMATCH';
  end if;
  ...
  v_plan := jsonb_build_object(
    ...
    'productCreates', p_apply->'productCreates',
    'variantCreates', p_apply->'variantCreates',
    'collectionCreates', p_apply->'collectionCreates',
  ...
  v_has_product_create := jsonb_array_length(p_apply->'productCreates') > 0;
  v_has_collection_create := jsonb_array_length(p_apply->'collectionCreates') > 0;
  ...
  if not v_has_product and not v_has_variant and not v_has_create and not v_has_product_create and not v_has_collection_create and not v_has_collection and not v_has_add and not v_has_remove then
  ...
  for v_item in
    select value from jsonb_array_elements(p_apply->'collectionCreates') as value
  loop
    ...
    if v_item->>'owner' is distinct from 'partner' then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    if v_item->>'status' is distinct from 'active' then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    if nullif(btrim(v_item->>'partnerId'), '') is distinct from v_partner_id then
      raise exception 'VIBODE_STAGE_PUBLISH:PARTNER_MISMATCH';
    end if;
    if v_item->>'partnerName' is distinct from v_partner_name then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    if v_partner_slug is not null and btrim(v_partner_slug) <> ''
       and v_collection_id not like ('col-' || v_partner_slug || '-%') then
      raise exception 'VIBODE_STAGE_PUBLISH:COLLECTION_NAMESPACE_MISMATCH';
    end if;
    if exists (
      select 1
      from public.vibode_stage_collections collections
      where collections.collection_id = v_collection_id
    ) then
      raise exception 'VIBODE_STAGE_PUBLISH:DUPLICATE_COLLECTION_ID';
    end if;
  ...
  for v_item in
    select value from jsonb_array_elements(p_apply->'productCreates') as value
  loop
    if jsonb_typeof(v_item) <> 'object' then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    v_product_id := nullif(btrim(v_item->>'productId'), '');
    v_default_variant_id := nullif(btrim(v_item->>'defaultVariantId'), '');
    if v_product_id is null or v_default_variant_id is null then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    if nullif(btrim(v_item->>'partnerId'), '') is distinct from v_partner_id then
      raise exception 'VIBODE_STAGE_PUBLISH:PARTNER_MISMATCH';
    end if;
    if v_item->>'source' is distinct from 'partner_catalog' then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    if v_item->>'status' is distinct from 'active' then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    if v_item->>'brand' is distinct from v_partner_name or v_item->>'retailer' is distinct from v_partner_name then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    if exists (
      select 1
      ...
      raise exception 'VIBODE_STAGE_PUBLISH:DUPLICATE_PRODUCT_ID';
    end if;
    select count(*)::integer
      into v_matched
    from jsonb_array_elements(p_apply->'variantCreates') as value
    where nullif(btrim(value->>'variantId'), '') is not distinct from v_default_variant_id
      and nullif(btrim(value->>'productId'), '') is not distinct from v_product_id
      and jsonb_typeof(value->'priceAmount') = 'number'
      and jsonb_typeof(v_item->'priceAmount') = 'number'
      and (value->>'priceAmount')::numeric is not distinct from (v_item->>'priceAmount')::numeric
      and nullif(btrim(value->>'priceCurrency'), '') is not distinct from nullif(btrim(v_item->>'priceCurrency'), '');
    if v_matched <> 1 then
      raise exception 'VIBODE_STAGE_PUBLISH:DEFAULT_VARIANT_MISMATCH';
    end if;
  end loop;
  ...
  loop
    ...
    if not found then
      raise exception 'VIBODE_STAGE_PUBLISH:PARTNER_ASSET_NOT_FOUND';
    end if;
    if v_asset_status is distinct from 'ready' then
      raise exception 'VIBODE_STAGE_PUBLISH:ASSET_NOT_READY';
    end if;
    if not exists (
      select 1
      from public.vibode_stage_partner_assets mappings
      where mappings.partner_id = v_partner_id
        and mappings.asset_id = v_asset_id
    ) then
      raise exception 'VIBODE_STAGE_PUBLISH:PARTNER_ASSET_UNMAPPED';
    end if;
  end loop;
  ...
  loop
    ...
    v_default_variant_id := nullif(btrim(v_item->>'defaultVariantId'), '');
    v_next_name := nullif(btrim(v_item->>'name'), '');
    v_next_image := nullif(btrim(v_item->>'imageUrl'), '');
    v_next_url := nullif(btrim(v_item->>'productUrl'), '');
    v_price_currency := nullif(btrim(v_item->>'priceCurrency'), '');
    v_category_id := nullif(btrim(v_item->>'categoryId'), '');
    v_subcategory_id := case
      when jsonb_typeof(v_item->'subcategoryId') = 'null' or v_item->'subcategoryId' is null then null
      when jsonb_typeof(v_item->'subcategoryId') = 'string' then nullif(btrim(v_item->>'subcategoryId'), '')
      else null
    end;
    if v_item ? 'subcategoryId' and jsonb_typeof(v_item->'subcategoryId') is distinct from 'null' and jsonb_typeof(v_item->'subcategoryId') is distinct from 'string' then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
    if v_product_id is null or v_default_variant_id is null or v_next_name is null or v_next_image is null or v_next_url is null or v_price_currency is null or v_category_id is null then
      raise exception 'VIBODE_STAGE_PUBLISH:INVALID_APPLY_PAYLOAD';
    end if;
...
revoke all on function public.vibode_stage_apply_partner_patch_v5(jsonb)
  from public, anon, authenticated;
grant execute on function public.vibode_stage_apply_partner_patch_v5(jsonb)
  to service_role;
