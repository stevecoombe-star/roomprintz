...
test("26) production AFC core stays isolated from diagnostics except AFD-2B analyze attach", () => {
  ...
  assert.doesNotMatch(
    analyze,
    /session-lifecycle|ensureAfcDiagnosticSessionMembership|qa-capability|retry-episode-signal|getAfcDiagnosticRetryEpisodeSignal|submit-tester-case|submitAfcDiagnosticTesterCase|browser-qa-state|getAfcDiagnosticBrowserQaState|vibode_afc_diagnostic_/,
  );
  const diagnosticsModule = [
    ...
    source("lib/afc-v2-diagnostics/qa-rerun.server.ts"),
  ].join("\n");
  assert.doesNotMatch(
    diagnosticsModule,
