...
import { createPi5dLoungeChairObject3D } from "@/lib/afc-v2-runtime/pi5d-lounge-chair-geometry";
...
async function main(): Promise<void> {
  const chair = createPi5dLoungeChairObject3D();
  const exporter = new GLTFExporter();
  const result = await exporter.parseAsync(chair, { binary: true });
