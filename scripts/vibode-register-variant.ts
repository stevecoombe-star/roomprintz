/**
 * PI-5E2 additional Variant registration CLI.
 *
 * Usage:
 *   npm run vibode:register-variant -- --input <variant.json>
 *   npm run vibode:register-variant -- --input <variant.json> --check
 */
...
function main(): void {
  ...
  if (!inputPath) {
    process.stderr.write("register-variant requires --input <variant.json>.\n");
  ...
  try {
    ...
    printJson({
      ...
      errors: [{
        code: "INVALID_JSON",
        message: error instanceof Error ? error.message : "Unable to read Variant JSON.",
      }],
    });
    process.exitCode = 1;
    return;
  }
  const parsed = parseVariantRegistrationJson(raw);
  if (!parsed.ok) {
    printJson({
      ok: false,
      check: flags.check === true,
