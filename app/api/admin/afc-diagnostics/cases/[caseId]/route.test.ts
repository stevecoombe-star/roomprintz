...
const ROUTE = "app/api/admin/afc-diagnostics/cases/[caseId]/route.ts";

test("admin diagnostics case detail route is GET-only Node runtime", () => {
  const route = readFileSync(path.join(ROOT, ROUTE), "utf8");
  assert.match(route, /export const runtime = "nodejs"/);
  assert.match(route, /handleAfcDiagnosticsAdminCaseDetailGet/);
